...
import {getAllProjects, getProjectByName, getProjectByStatus, getProjectByCraft, createNewProject} from "../models/projects.js";
...
router.get ("/", async function (req, res) {
    ...
    if (project_name) {
        ...
        if (projectByName.length === 0){
            return res.json({success: false, payload: `There is no project with the name ${project_name}`})
            } else {
    ...
    if (status) {
        ...
        if (projectByStatus !== undefined) {
            return res.json({success: true, payload: projectByStatus});
                ...
                return res.json({success: false, payload: `There is no project with the status ${status}`})}};
    ...
    if (craft) {
        ...
        if (projectByCraft !== undefined) {
            return res.json({success: true, payload: projectByCraft});
                ...
                return res.json({success: false, payload: `There is no project with the craft ${craft}`})}};
